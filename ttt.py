...
if is_causal_conv1d_available():
    ...
    import时报错 No module named 'causal_conv1d_cuda'
    https://blog.csdn.net/weixin_51005487/article/details/139382288
    """
...
class TTTCache_MK2(nn.Module): # 复制黏贴后简化（因为只想用TTTLinear)

    def __init__(self, model, batch_size, mini_batch_size: int):
        super().__init__()
        ...
        match model_class:
            ...
            case "TTTMLP":
                ...
                dict_weight = {"W1": model.W1, "b1":model.b1, "W2":model.W2, "b2":model.b2}
    ...
    def update(self, py_tree, layer_idx, seq_len):
        if seq_len % self.mini_batch_size == 0:
            ...
            for name in self.ttt_param_names:
                if self.ttt_params_dict[f"{name}_states"].shape != py_tree[f"{name}_states"].shape:
                    py_tree[f"{name}_states"] = fulfill(py_tree[f"{name}_states"], self.ttt_params_dict[f"{name}_states"])
                self.ttt_params_dict[f"{name}_states"].copy_(py_tree[f"{name}_states"])
                # self.ttt_params_dict[f"{name}_states"].copy_(py_tree[f"{name}_states"].clone())
...
def scan(f, init, xs, out, checkpoint_group=0): # 这个函数？？？ 需要明确这里的for循环？ 时间维度的循环到底在哪？？？
    ...
    def __init__(self, model, batch_size: int):
        ...
        for layer_idx in range(config.num_hidden_layers):
            for name in self.ttt_param_names:
                ...
                self.ttt_params_dict[f"{name}_grad"][layer_idx] = torch.zeros_like(tiled_weight).to(model.device)
...
class TTTBase(nn.Module):
    ...
    def forward(
        ...
        del XQ, XK, XV, output_mod, last_mini_batch_params_dict
    ...
    def ttt(
        ...
        def compute_mini_batch(params_dict, inputs):
            ...
            if use_dual_form:
                ...
                grad_W1 = torch.einsum("bhnk,bhkij->bhnij", torch.tril(ttt_lr_eta_mini_batch), grad_W1.detach())
            ...
            Z1_bar = ln_fwd(Z1_bar.detach(), ln_weight, ln_bias)
            ...
            for var in list(locals().keys()):
                if var not in ['result', 'XQW_mini_batch']:
                    del locals()[var]
        ...
        keep_vars = [XQW_batch, batch_params_dict]
        for var in list(locals().keys()):
...
class TTTMLP(TTTBase):
    ...
    def ttt(
        ...
        def compute_mini_batch(params_dict, inputs):
            ...
            if use_dual_form:
                Attn1 = torch.tril(XQ_mini_batch @ X1.transpose(-2, -1)).detach()  # [B,nh,K,K]
                ...
                grad_W2 = torch.einsum("bhnk,bhkij->bhnij", torch.tril(ttt_lr_eta_mini_batch), grad_W2.detach())
            ...
            Z2_bar = ln_fwd(Z2_bar.detach(), ln_weight, ln_bias)

            XQW_mini_batch = XQ_mini_batch + Z2_bar

            # last_param_dict = { # 这个加法效果很差 这个位置太早了 应该再晚一些detach
            #     "W1_states": W1_last.detach(),
            #     "b1_states": b1_last.detach(),
            #     "W2_states": W2_last.detach(),
            #     "b2_states": b2_last.detach(),
            #     "W1_grad": grad_W1_last.detach(),
            #     "b1_grad": grad_b1_last.detach(),
            #     "W2_grad": grad_W2_last.detach(),
            #     "b2_grad": grad_b2_last.detach(),
            # }
            last_param_dict = {
                "W1_states": W1_last,
                "b1_states": b1_last,
            ...
            keep_vars = [last_param_dict, XQW_mini_batch]
            for var in list(locals().keys()):
                if var not in keep_vars:
                    del locals()[var]
            return last_param_dict, XQW_mini_batch
        ...
        # XQW_batch: [num_mini_batch, B, num_heads, mini_batch_size, head_dim] 应该是在这里的某个地方打断？
        batch_params_dict, XQW_batch = scan(
            compute_mini_batch,
            init_params_dict,
            inputs, 
        ...
        # for key in batch_params_dict: # 在运算结束时强制打断梯度 # 这里太晚了
        #     batch_params_dict[key] = batch_params_dict[key].detach()
        # XQW_batch = XQW_batch.detach()
...
if __name__ == "__main__":
    ...
    config = TTTConfig(
        ...
        intermediate_size=4*hidden_size,    # MLP中间层的大小，可以设置为hidden_size的倍数
