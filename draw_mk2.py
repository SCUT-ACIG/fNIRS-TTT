...
def draw_all():
    ...
    # 创建画布和网格布局
    fig = plt.figure(figsize=(18, 6))  # 增加高度以适应上下布局
    
    # 创建网格规格：1行2列，右边再分为2行
    gs = gridspec.GridSpec(1, 2, width_ratios=[1, 2])  # 左右两列宽度比为1:1
    gs_right = gridspec.GridSpecFromSubplotSpec(2, 1, subplot_spec=gs[1])  # 右边区域分为2行

    # 创建子图
    axes = [
        fig.add_subplot(gs[0]),           # 左边的方形图 (Dataset A)
        fig.add_subplot(gs_right[0]),     # 右上的长方形图 (Dataset B)
        fig.add_subplot(gs_right[1])      # 右下的长方形图 (Dataset C)
    ]
    ...
    for i, dataset in enumerate(dataset_num):
        ...
        for j, model in enumerate(models):
            ...
            # 绘制折线
            if model == "fNIRS_TTT_M": 
                model = "fNIRS-TTT-M"
            if model == "fNIRS_TTT_L": 
                model = "fNIRS-TTT-L"
            if model == "CT-Net-old": 
                model = "CT-Net"
    ...
    # 调整布局并保存图像
    plt.tight_layout()
    plt.savefig("loso_accuracy_all_datasets_mk2.tiff")
    plt.savefig("loso_accuracy_all_datasets_mk2.png", dpi=300)
    plt.show()
